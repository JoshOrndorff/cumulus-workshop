...
> 如果您打算让其他人链接到您的网路，**您必须**链条规格必须有genesis Wasm 
...
这个工作间包含三个链规范文件，您可以**不用修改**就可以使用它们：
...
与任何 Substrate 链一样，您始终可以创建自己的链规范文件。一般最好开始
从现有规范中，因为从头开始开发超出了本教程的范围。
对此感兴趣的人，请参阅下面的 [Further Resources](#chain-specification)  部分。
现在，我们将使用 Polkadot 的内置 `rococo-local` 作为我们自定义规范的起点。
...
该文件包含我们已经需要的大部分信息。Rococo是一个许可链，所以
我们只需要添加一个权限及其会话密钥。下面的片段显示了的相关部分
生成的规范文件。生成的文件中的所有密钥都属于常用的知名帐户
在其他教程中（Alice 和 Bob 在 `rococo-custom-plain.json` 文件的情况下）。
...
在这个运行时配置中，两个IDs都是一样的，而且都是从“stash”账户生成的。您可以生成您自己的账户或检查[well-known development accounts](https://substrate.dev/docs/en/knowledgebase/integrate/subkey#well-known-keys).
...
`//Alice`（`sr25519` 密码学）的 Polkadot 地址。这用于除“beefy”之外的所有
`grandpa` 键之后的 chainspec 的关键部分
...
ECDSA 钥匙有所不同[the note below](#ss58-encoding-of-key-vs-address)的原因).
